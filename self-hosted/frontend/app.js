...
class ExpertReviewApp {
  ...
  setAnalysisLoading(isLoading) {
    ...
    const submitBtn = querySelector('#analysis-form button[type="submit"]');

    if (submitBtn) {
      if (isLoading) {
        submitBtn.classList.add(CONFIG.CLASSES.LOADING);
        submitBtn.disabled = true;
      } else {
        submitBtn.classList.remove(CONFIG.CLASSES.LOADING);
